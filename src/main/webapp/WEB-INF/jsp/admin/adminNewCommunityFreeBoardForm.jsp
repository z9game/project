...
<script>
	function checkCommunityFreeBoardForm() {
		...
		$.ajax({
				url : "/adminCommunityFreeBoardRegProc.do"
			,	type : "post"
			,	data : serialize
			,	success : function(json) {				
					var result = json["result"];					
					if (result == 1) {
						alert("관리자 자유게시판 등록 성공 입니다.");
						location.replace("/adminFreeBoardForm.do")
					} 
					else {
						alert("관리자 자유게시판 등록 실패입니다!");
					}
				}
			,	error : function() {				
					alert("관리자 자유게시판 등록 실패! 관리자에게 문의 바랍니다.");
				}
		});
			...
			<div class="newCommunityFreeBoardFormBtnDiv">
				...
				<div class="boardRegAndMoveList">
					<input type="button" class="boardRegBtn" value="저장"
						onClick="checkCommunityFreeBoardForm();"> 
						<input type="button" class="moveListBtn" value="목록"
